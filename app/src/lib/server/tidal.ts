 ...
 */
export function getTidalImageUrl(picId?: string | null, size: "1280x1280" | "750x750" | "320x320" = "1280x1280"): string {
	if (!picId) return "";
 ...
 * Search Tidal tracks
 */
export async function searchTidalTracks(query: string, limit = 25): Promise<Track[]> {
	...
	try {
		...
		return items.map((item: any, i: number): Track => {
			...
			const isLossless = item.audioQuality === "LOSSLESS" || item.audioQuality === "HIRES_LOSSLESS";

			return {
				id: `tidal_${item.id}`,
				title: item.title || "Unknown Title",
				artist: item.artist?.name || item.artists?.map((a: any) => a.name).join(", ") || "Unknown Artist",
				album: item.album?.title || "Single",
				genre: item.mediaMetadata?.tags?.[0] || "Pop",
				year: item.streamStartDate ? new Date(item.streamStartDate).getFullYear() : new Date().getFullYear(),
 ...
 */
export async function searchTidalArtists(query: string, limit = 10): Promise<SearchArtist[]> {
	const token = await getTidalAccessToken();
	if (!token) return [];
	...
	try {
		...
		return items.map((item: any): SearchArtist => ({
			...
			image: getTidalImageUrl(item.picture, "750x750") || "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&auto=format&fit=crop&q=80",
			role: "Artist",
 ...
 * Search Tidal albums
 */
export async function searchTidalAlbums(query: string, limit = 10): Promise<SearchAlbum[]> {
	const token = await getTidalAccessToken();
	if (!token) return [];
	...
	try {
		...
		return items.map((item: any): SearchAlbum => ({
			...
			artist: item.artist?.name || item.artists?.map((a: any) => a.name).join(", ") || "Various Artists",
 ...
 */
export async function searchTidalPlaylists(query: string, limit = 10): Promise<SearchPlaylist[]> {
	const token = await getTidalAccessToken();
	if (!token) return [];
 ...
 */
export async function getTidalAlbumDetails(albumId: string | number): Promise<{ album: SearchAlbum; tracks: Track[] } | null> {
	const token = await getTidalAccessToken();
	...
	try {
		const albumRes = await fetch(`${TIDAL_API_BASE}/albums/${cleanId}?countryCode=US`, {
			headers: { Authorization: `Bearer ${token}` },
		});
		...
		const { searchJioSaavnAlbums, getJioSaavnAlbumDetails } = await import("./music");
		const saavnAlbs = await searchJioSaavnAlbums(`${albumData.title} ${artistName}`, 3);
			...
			if (fullAlb && fullAlb.tracks.length > 0) {
				return {
					album: {
						...
						year: albumData.releaseDate ? new Date(albumData.releaseDate).getFullYear() : fullAlb.album.year,
		...
		const searchTracks = await searchTidalTracks(`${albumData.title} ${artistName}`, albumData.numberOfTracks || 25);
		const album: SearchAlbum = {
			...
			year: albumData.releaseDate ? new Date(albumData.releaseDate).getFullYear() : undefined,
			tracks: searchTracks,
		};
 ...
 */
export async function getTidalArtistTopTracks(artistId: string | number, limit = 25): Promise<{ artist: SearchArtist; tracks: Track[] } | null> {
	const token = await getTidalAccessToken();
	if (!token) return null;
	...
	try {
		const artistRes = await fetch(`${TIDAL_API_BASE}/artists/${cleanId}?countryCode=US`, {
			headers: { Authorization: `Bearer ${token}` },
		});
 ...
 */
export async function getTidalTrack(trackId: string | number): Promise<any | null> {
	const token = await getTidalAccessToken();
	if (!token) return null;
	...
	try {
		const res = await fetch(`${TIDAL_API_BASE}/tracks/${cleanId}?countryCode=US`, {
			headers: { Authorization: `Bearer ${token}` },
		});
...
function cleanTrackTitle(t: string): string {
	return t
		.replace(/\s*[\(\[][^\)\]]*(feat|ft|with|remix|version|deluxe|edit|explicit|remaster)[^\)\]]*[\)\]]/gi, "")
 ...
 */
export async function resolveTidalTrackStream(trackId: string | number): Promise<string | null> {
			...
			if (matchedTracks.length > 0) {
				...
				const bestMatch =
					matchedTracks.find((m) => {
						...
						const mArtist = (m.artist || "").toLowerCase().replace(/[^a-z0-9]/g, "");
						const tMatch = mTitle === normTitle || mTitle.includes(normTitle) || normTitle.includes(mTitle);
						const aMatch = !normArtist || mArtist.includes(normArtist) || normArtist.includes(mArtist);
						return tMatch && aMatch;
					}) ||
					matchedTracks.find((m) => {
						const mTitle = m.title.toLowerCase().replace(/[^a-z0-9]/g, "");
						return mTitle === normTitle || (normTitle.length >= 5 && (mTitle.includes(normTitle) || normTitle.includes(mTitle)));
					}) ||
					matchedTracks[0];
