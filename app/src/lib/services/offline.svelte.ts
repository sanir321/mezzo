...
class OfflineStore {
	...
	private saveMetadata() {
		if (typeof window === "undefined") return;
		try {
			localStorage.setItem(STORAGE_KEY_OFFLINE_TRACKS, JSON.stringify(this.downloadedTracks));
		} catch (e) {
	...
	async downloadTrack(track: Track): Promise<boolean> {
		...
		try {
			...
			const audioReq = new Request(audioUrl, { mode: "cors" });
			const audioRes = await fetch(audioReq);
			if (!audioRes.ok) {
				throw new Error(`Failed to fetch audio stream: status ${audioRes.status}`);
			}
			...
			this.downloadedTracks = [track, ...this.downloadedTracks.filter((t) => t.id !== track.id)];
			this.saveMetadata();
			return true;
		} catch (err) {
