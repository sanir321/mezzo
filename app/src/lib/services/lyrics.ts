...
function scoreHit(hit: LrcHit, trackQ: string, artistQ: string): number {
	...
	const aScore = artistQ ? jaccard(tokenSet(stripFeat(hit.artistName || "")), artistMatcher) : 0;
...
function pickBestHit(hits: unknown[], trackName: string, cleanArtist: string): LrcHit | null {
