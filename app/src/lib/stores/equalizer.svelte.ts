...
class EqualizerStore {
  ...
  private loadState() {
    try {
      ...
      if (saved) {
        ...
        const hasCustomGains =
          Array.isArray(data.gains) && data.gains.some((g: number) => g !== 0);
        const isNonFlat =
          typeof data.currentPreset === "string" &&
          data.currentPreset !== "Flat";
        ...
        if (typeof data.currentPreset === "string")
  ...
  attachAudioElement(audioEl: HTMLAudioElement) {
    ...
    if (
      !this.enabled ||
      (this.currentPreset === "Flat" && this.bands.every((b) => b.gain === 0))
    ) {
      return;
    }
